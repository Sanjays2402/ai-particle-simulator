...
export function generateRandomScene(presetIds, opts = {}) {
  ...
  const force   = pick(FORCE_FIELDS, rng)

  // Build the scene field-by-field. Clamps mirror the store setters'
  // clamps so applyScene later doesn't need to re-clamp.
  const scene = {
    currentPreset: pick(presetIds, rng),
    particleCount: rstep(5000, 40000, 1000, rng),
    speed:         Number(rrange(0.3, 2.5, rng).toFixed(2)),
    glowIntensity: Number(rrange(0.1, 0.9, rng).toFixed(2)),
    visualStyle:   pick(STYLES, rng),
    theme:         pick(BUILTIN_THEMES, rng),
    trails:        chance(0.5, rng),
    mouseAttract:  chance(0.3, rng),
    attractStrength: Number(rrange(0.5, 4.0, rng).toFixed(2)),
    ...
    chromaticAberration: chance(0.4, rng),
    chromaticIntensity:  Number(rrange(0.001, 0.008, rng).toFixed(4)),
    vignette:            chance(0.5, rng),
    vignetteIntensity:   Number(rrange(0.3, 0.7, rng).toFixed(2)),
    filmGrain:           chance(0.3, rng),
    filmGrainIntensity:  Number(rrange(0.1, 0.3, rng).toFixed(2)),

    // Kaleidoscope is high-impact so it's gated low.
    kaleidoscopeEnabled: chance(0.25, rng),
    kaleidoscopeSegments: Math.floor(rrange(2, 16, rng)),

    // Hue cycle is a slow accent so it's safe at higher chance.
    hueCycleEnabled: chance(0.4, rng),
    ...
    bgGradientEnabled: chance(0.45, rng),
    bgGradientA:       bg[0],
    bgGradientB:       bg[1],
    bgGradientAngle:   Math.floor(rrange(0, 360, rng)),
    bgGradientAudioReactive: chance(0.5, rng),
